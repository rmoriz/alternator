...
async fn test_config_environment_variable_overrides() {
    ...
    std::env::set_var("ALTERNATOR_MASTODON_INSTANCE_URL", "https://env.mastodon.social");
...
async fn test_config_validation_missing_required_fields() {
    // Test missing mastodon instance URL
    let mut config = create_test_config();
    config.mastodon.instance_url = String::new();
...
async fn test_application_component_initialization() {
    ...
    let _openrouter_client = alternator::openrouter::OpenRouterClient::new(config.openrouter.clone());
...
async fn test_error_handling_and_recovery() {
    ...
    let openrouter_rate_limit = AlternatorError::OpenRouter(
        OpenRouterError::RateLimitExceeded { retry_after: 60 }
    );
    assert!(ErrorRecovery::is_recoverable(&openrouter_rate_limit));
    
    // Test non-recoverable errors
    let config_error = AlternatorError::Config(
        alternator::config::ConfigError::MissingRequired("test".to_string())
    );
...
async fn test_media_processing_pipeline() {
    ...
    let prompt = language_detector.get_prompt_template(&detected_lang).unwrap();
...
async fn test_balance_monitoring_configuration() {
    ...
    let openrouter_client = alternator::openrouter::OpenRouterClient::new(
        OpenRouterConfig {
    ...
    let openrouter_client2 = alternator::openrouter::OpenRouterClient::new(
        OpenRouterConfig {
            api_key: "test_key".to_string(),
            model: "test_model".to_string(),
            base_url: None,
