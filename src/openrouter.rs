...
impl OpenRouterClient {
    ...
    fn base_url(&self) -> &str {
        self.config.base_url.as_deref().unwrap_or("https://openrouter.ai/api/v1")
    }
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        if status == 429 {
            ...
            warn!("OpenRouter rate limit exceeded, retry after {} seconds", retry_after);
        ...
        if !status.is_success() {
            ...
            if let Ok(error_response) = serde_json::from_str::<ErrorResponse>(&response_text) {
                ...
                if error_msg.to_lowercase().contains("token") && error_msg.to_lowercase().contains("limit") {
                ...
                if error_msg.to_lowercase().contains("balance") || error_msg.to_lowercase().contains("credit") {
        ...
        loop {
            ...
            match self.handle_response::<T>(response).await {
                ...
                Err(e) => {
                    ...
                    warn!("API request failed (attempt {}): {}, retrying in {}ms", 
                          attempt + 1, e, delay);
    ...
    pub async fn get_account_balance(&self) -> Result<f64, OpenRouterError> {
        ...
        let response: AccountBalance = self
            .api_request_with_retry(
                || self.http_client.get(&format!("{}/auth/key", self.base_url())),
                3,
            )
            .await?;
    ...
    pub async fn list_models(&self) -> Result<Vec<Model>, OpenRouterError> {
        ...
        if !model_available {
            warn!("Configured model '{}' not found in available models", configured_model);
    ...
    pub async fn describe_image(&self, image_data: &[u8], prompt: &str) -> Result<String, OpenRouterError> {
        debug!("Generating image description using model: {}", self.config.model);
...
mod tests {
    ...
    fn test_image_description_response_deserialization() {
        ...
        assert_eq!(response.choices[0].message.content, "A beautiful sunset over the ocean");
    ...
    async fn test_image_size_validation() {
        ...
        let result = client.describe_image(&large_image, "Describe this image").await;
    ...
    fn test_openrouter_error_display() {
        ...
        assert!(format_error.to_string().contains("Unsupported image format"));
