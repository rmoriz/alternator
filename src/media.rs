...
impl Default for MediaConfig {
    fn default() -> Self {
        Self {
            ...
            supported_formats: SUPPORTED_IMAGE_FORMATS.iter().map(|s| s.to_string()).collect(),
        }
    }
}
    ...
    fn check_size_limits(&self, data: &[u8]) -> Result<(), MediaError> {
        ...
        if size_mb > self.config.max_size_mb {
            return Err(MediaError::ProcessingFailed(
                format!("Image size {:.2}MB exceeds limit of {:.2}MB", 
...
impl MediaTransformer for ImageTransformer {
    fn is_supported(&self, media_type: &str) -> bool {
        self.config.supported_formats.contains(&media_type.to_lowercase())
    }
        ...
        match output_format {
            ImageFormat::Png => {
                ...
                resized_img.write_with_encoder(encoder)
                    .map_err(|e| MediaError::EncodingFailed(format!("Failed to encode PNG: {}", e)))?;
            }
            ImageFormat::Jpeg => {
                let encoder = JpegEncoder::new_with_quality(&mut output, 85);
                resized_img.write_with_encoder(encoder)
                    .map_err(|e| MediaError::EncodingFailed(format!("Failed to encode JPEG: {}", e)))?;
            }
            _ => {
                // Fallback to PNG for other formats
                let encoder = PngEncoder::new(&mut output);
                resized_img.write_with_encoder(encoder)
                    .map_err(|e| MediaError::EncodingFailed(format!("Failed to encode fallback PNG: {}", e)))?;
            }
        }
...
impl MediaProcessor {
    ...
    pub fn filter_processable_media<'a>(&self, media_attachments: &'a [MediaAttachment]) -> Vec<&'a MediaAttachment> {
        media_attachments
            .iter()
            .filter(|media| {
    ...
    pub async fn process_media_for_analysis(&self, media: &MediaAttachment) -> Result<Vec<u8>, MediaError> {
    ...
    pub fn get_media_stats(&self, media_attachments: &[MediaAttachment]) -> MediaStats {
        ...
        let supported = media_attachments.iter()
            .filter(|m| self.transformer.is_supported(&m.media_type))
            .count();
        let needs_description = media_attachments.iter()
            .filter(|m| self.transformer.is_supported(&m.media_type) && self.transformer.needs_description(m))
            .count();
