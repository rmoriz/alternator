...
mod tests {
    ...
    fn test_mastodon_error_variants() {
        ...
        assert!(connection_error.to_string().contains("WebSocket connection failed"));
        ...
        let toot_not_found = MastodonError::TootNotFound { toot_id: "123".to_string() };
    ...
    fn test_openrouter_error_variants() {
        ...
        assert!(token_limit_error.to_string().contains("Token limit exceeded"));
    ...
    fn test_media_error_variants() {
        ...
        assert!(unsupported_error.to_string().contains("Unsupported media type"));
    ...
    fn test_language_error_variants() {
        ...
        assert!(detection_error.to_string().contains("Language detection failed"));
        
        let unsupported_error = LanguageError::UnsupportedLanguage { 
            language: "xyz".to_string() 
        };
        assert!(unsupported_error.to_string().contains("Unsupported language: xyz"));
        
        let template_error = LanguageError::PromptTemplateNotFound { 
            language: "fr".to_string() 
        };
        assert!(template_error.to_string().contains("Prompt template not found"));
    ...
    fn test_error_recovery_is_recoverable() {
        ...
        let websocket_error = AlternatorError::WebSocket(
            tokio_tungstenite::tungstenite::Error::ConnectionClosed
        );
        assert!(ErrorRecovery::is_recoverable(&websocket_error));
        
        let mastodon_connection_error = AlternatorError::Mastodon(
            MastodonError::ConnectionFailed("timeout".to_string())
        );
        assert!(ErrorRecovery::is_recoverable(&mastodon_connection_error));
        
        let mastodon_rate_limit = AlternatorError::Mastodon(
            MastodonError::RateLimitExceeded { retry_after: 60 }
        );
        assert!(ErrorRecovery::is_recoverable(&mastodon_rate_limit));
        
        let openrouter_rate_limit = AlternatorError::OpenRouter(
            OpenRouterError::RateLimitExceeded { retry_after: 30 }
        );
        assert!(ErrorRecovery::is_recoverable(&openrouter_rate_limit));
        
        // Non-recoverable errors
        let config_error = AlternatorError::Config(
            ConfigError::MissingRequired("test".to_string())
        );
        assert!(!ErrorRecovery::is_recoverable(&config_error));
        
        let mastodon_auth_error = AlternatorError::Mastodon(
            MastodonError::AuthenticationFailed("invalid token".to_string())
        );
        assert!(!ErrorRecovery::is_recoverable(&mastodon_auth_error));
        
        let openrouter_auth_error = AlternatorError::OpenRouter(
            OpenRouterError::AuthenticationFailed
        );
        assert!(!ErrorRecovery::is_recoverable(&openrouter_auth_error));
        
        let token_limit_error = AlternatorError::OpenRouter(
            OpenRouterError::TokenLimitExceeded { tokens_used: 200, max_tokens: 150 }
        );
        assert!(!ErrorRecovery::is_recoverable(&token_limit_error));
        
        let media_error = AlternatorError::Media(
            MediaError::UnsupportedType { media_type: "video/mp4".to_string() }
        );
    ...
    fn test_nested_error_conversion() {
        ...
        let mastodon_error = MastodonError::TootNotFound { toot_id: "123".to_string() };
